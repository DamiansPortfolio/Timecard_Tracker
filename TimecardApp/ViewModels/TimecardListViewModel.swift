...
//
//class TimecardListViewModel: ObservableObject {
//    // MARK: - Published Properties for Timecards
//    @Published var timecards: [Timecard] = []
//    @Published var filteredTimecards: [Timecard] = []
//    @Published var currentFilter: TimecardStatus? = nil
//
//    // MARK: - Published Properties for Leave Requests
...
//                    }
//
//                    self.leaveRequests = querySnapshot?.documents.compactMap { LeaveRequest(document: $0) } ?? []
...
//    }
//
//    // MARK: - Filter Timecards
//    func filterByStatus(_ status: TimecardStatus?) {
//        currentFilter = status
//        filteredTimecards = status == nil ? timecards : timecards.filter { $0.status == status }
//    }
//
//    // MARK: - Filter Leave Requests
//    func filterLeaveRequests(by status: LeaveStatus?) {
//        currentLeaveFilter = status
//        filteredLeaveRequests = status == nil ? leaveRequests : leaveRequests.filter { $0.status == status }
//    }
//

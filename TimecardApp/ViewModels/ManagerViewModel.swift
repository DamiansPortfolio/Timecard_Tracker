...
////
////                    if let error = error {
////                        print("Error fetching timecards for employee \(employeeId): \(error.localizedDescription)")
////                        return
////                    }
////
////                    let timecards = snapshot?.documents.compactMap { Timecard(document: $0) } ?? []
////                    let weeklyHours = timecards.filter {
...
class ManagerViewModel: ObservableObject {
    ...
    func fetchManagerData(managerId: String) {
        ...
        db.collection("managers")
            ...
            .getDocuments { [weak self] managerSnapshot, error in
                ...
                self.fetchManagedEmployeesProfiles(employeeIds: employeeIds)
                self.fetchPendingApprovals(employeeIds: employeeIds)
                self.fetchPendingLeaveRequests(employeeIds: employeeIds)
            }
    }
    ...
    // MARK: - Fetch Pending Leave Requests
    private func fetchPendingLeaveRequests(employeeIds: [String]) {
        db.collection("leaveRequests")
            .whereField("userId", in: employeeIds)
            .whereField("status", isEqualTo: LeaveStatus.pending.rawValue)
            .getDocuments { [weak self] snapshot, error in
...
//
//                if let error = error {
//                    self.handleError("Error fetching manager data: \(error.localizedDescription)")
//                    return
//                }
//
//                guard let managerDoc = managerSnapshot?.documents.first,
...
//        guard let startOfWeek = calendar.dateInterval(of: .weekOfYear, for: Date())?.start,
//              let endOfWeek = calendar.date(byAdding: .day, value: 7, to: startOfWeek) else {
//            handleError("Unable to determine the start of the week.")
//            return
//        }
//
//        let group = DispatchGroup()
...
//    }
//
//    func fetchPendingApprovals(employeeIds: [String]) {
//        isLoading = true
//
//        db.collection("timecards")
//            .whereField("userId", in: employeeIds)
...
//                }
//
//                self.pendingLeaveRequests = snapshot?.documents.compactMap {
//                    LeaveRequest(document: $0, firstNameKey: "fname", lastNameKey: "lname")
//                } ?? []
//
//                self.isLoading = false
//            }
